...
def load_imp(hazard):
    ...
    # tree interpreter
    if hazard == 'road_surface':
        ti_fname = join(other_base_path,f'ti_rank_{hazard}_original.nc')
        ti_rank = explainer.load(ti_fname)

    
    # lime
    lime_fname = join(other_base_path,f'lime_rank_{hazard}_original.nc')
    lime_rank = explainer.load(lime_fname)

    
    if hazard == 'road_surface':
        return ([bsp, bmp, fsp, fmp, gini_rank, shap_rank, ale_var, lime_rank, ti_rank],
                ['singlepass', 'multipass', 'singlepass', 'multipass', 'gini', 
                 'shap_sum', 'ale_variance', 'lime', 'tree_interpreter'], name)
                
    else:
        return ([bsp, bmp, fsp, fmp, coef_rank, shap_rank, ale_var, lime_rank],
                ['singlepass', 'multipass', 'singlepass', 'multipass', 'coefs', 
                 'shap_sum', 'ale_variance', 'lime'], name)